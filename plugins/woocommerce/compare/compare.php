...
define("COMPARE_WOO_PLUGIN_URL", WOO_PLUGIN_URL . "/compare");
define("COMPARE_COOKIE_NAME", "as_compare_products");
/* ----------------------------------------------------------------------------------- */
/* Ajax   */
/* ----------------------------------------------------------------------------------- */
add_ajax("as_add_compare", "as_ajax_add_compare_product");
add_ajax("as_remove_compare", "as_ajax_remove_compare_product");
/* ----------------------------------------------------------------------------------- */
/* Filters   */
/* ----------------------------------------------------------------------------------- */
...
/* ShortCodes   */
/* ----------------------------------------------------------------------------------- */
add_shortcode('as_compare', 'as_shortcode_page_compare');

function as_compare_scripts() {
    add_script('compare', COMPARE_WOO_PLUGIN_URL . '/js/compare.js', array(
        'jquery'));
}

function as_shortcode_page_compare() {
    ...
    if (!empty($product_id) && is_numeric($product_id)) {
        ...
        if (!empty($product)) {
            ...
            if (isset($compare_product[$product_id])) {
                $response["message"] = '<div class="as-tooltip-wishlist-compare">' . __("It's available,", 'alenastudio') . '  <a target="_blank" href="' . get_page_link(get_option("as-compare-page-id")) . '">' . __("Go to Compare", 'alenastudio') . '</a>' . '</div>';
            }
            else {
                ...
                $response     = array(
                    "status"  => 1,
                    "message" => "Success",
                    "notice"  => '<div class="as-tooltip-wishlist-compare">' . __("It's available,", 'alenastudio') . '  <a target="_blank" href="' . get_page_link(get_option("as-compare-page-id")) . '">' . __("Go to Compare", 'alenastudio') . '</a>' . '</div>'
                );
            }
        }
        else {
...
function as_save_product_compare($product) {
    ...
    as_add_cookie(COMPARE_COOKIE_NAME, $data_product);
}
...
function as_get_product_compare() {
    return as_get_cookie(COMPARE_COOKIE_NAME);
}
...
function as_after_add_to_cart_button() {
    ...
    if (isset($data_product[$product->id])) {
        echo '<div class="compare-notice">
        		<div class="as-tooltip-wishlist-compare">' . __("It's available,", 'alenastudio') . '<a target="_blank" href="' . get_page_link(get_option("as-compare-page-id")) . '">' . __("Go to Compare!", 'alenastudio') . '</a></div>
