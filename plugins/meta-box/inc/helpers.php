...
//                'shortcode'));
        ...
        static function shortcode($atts) {
            ...
            if (in_array($atts['type'], array(
                ...
                foreach ($meta as
                        $file) {
                    $content .= sprintf(
                            '<li><a href="%s" title="%s">%s</a></li>', $file['url'], $file['title'], $file['name']
                    );
                }
                $content .= '</ul>';
            }
            ...
            elseif (in_array($atts['type'], array(
                ...
                foreach ($meta as
                    ...
                    if (isset($atts['link']) && $atts['link']) {
                        $content .= sprintf(
                                '<li><a href="%s" title="%s"><img src="%s" alt="%s" title="%s" /></a></li>', $image['full_url'], $image['title'], $image['url'], $image['alt'], $image['title']
                        );
                    }
                    else {
                        $content .= sprintf(
                                '<li><img src="%s" alt="%s" title="%s" /></li>', $image['url'], $image['alt'], $image['title']
                        );
                    }
                }
                $content .= '</ul>';
            ...
            elseif ('taxonomy' == $atts['type']) {
                ...
                foreach ($meta as
                        $term) {
                    $content .= sprintf(
                            '<li><a href="%s" title="%s">%s</a></li>', get_term_link($term, $atts['taxonomy']), $term->name, $term->name
                    );
                }
                $content .= '</ul>';
            }
        ...
        static function map($key, $args = array(), $post_id = null) {
            ...
            $html = sprintf(
                    '<div id="rwmb-map-canvas-%d" style="width:%s;height:%s"></div>', $counter, $args['width'], $args['height']
            );
            ...
            $html .= sprintf('
				...
				', $parts[0], $parts[1], json_encode($args['js_options']), $counter
            );
...
function rwmb_get_field($key, $post_id = null) {
    ...
    $field = call_user_func(array(
        RW_Meta_Box::get_class_name($field),
