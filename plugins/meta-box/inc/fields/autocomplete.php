...
if (!class_exists('RWMB_Autocomplete_Field')) {
    ...
    class RWMB_Autocomplete_Field extends RWMB_Field {
        ...
        static function admin_enqueue_scripts() {
            wp_enqueue_style('rwmb-autocomplete', RWMB_CSS_URL . 'autocomplete.css', array(
                'wp-admin'), RWMB_VER);
            wp_enqueue_script('rwmb-autocomplete', RWMB_JS_URL . 'autocomplete.js', array(
                'jquery-ui-autocomplete'), RWMB_VER, true);
            wp_localize_script('rwmb-autocomplete', 'RWMB_Autocomplete', array(
        ...
        static function html($meta, $field) {
            ...
            $html = sprintf(
                    '<input type="text" class="rwmb-autocomplete" id="%s" data-name="%s" data-options="%s" size="%s">', $field['id'], $field['field_name'], esc_attr(json_encode($options)), $field['size']
            );
            ...
            foreach ($field['options'] as
                ...
                if (in_array($value, $meta)) {
                    $html .= sprintf(
                            $tpl, $label, __('Delete', 'meta-box'), $field['field_name'], $value
                    );
                }
            }
