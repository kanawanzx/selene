...
if (!class_exists('RWMB_Time_Field')) {
    ...
    class RWMB_Time_Field extends RWMB_Field {
        ...
        static function admin_enqueue_scripts() {
            ...
            wp_enqueue_script('rwmb-time', RWMB_JS_URL . 'time.js', array(
                'jquery-ui-timepicker-i18n'), RWMB_VER, true);
        ...
        static function html($meta, $field) {
            return sprintf(
                    '<input type="text" class="rwmb-time" name="%s" value="%s" id="%s" size="%s" data-options="%s">', $field['field_name'], $meta, isset($field['clone']) && $field['clone'] ? '' : $field['id'], $field['size'], esc_attr(json_encode($field['js_options']))
            );
        }
