...
if (!class_exists('RWMB_Select_Advanced_Field')) {
    ...
    class RWMB_Select_Advanced_Field extends RWMB_Select_Field {
        ...
        static function admin_enqueue_scripts() {
            ...
            wp_enqueue_script('rwmb-select-advanced', RWMB_JS_URL . 'select-advanced.js', array(
                'select2'), RWMB_VER, true);
        }
        ...
        static function html($meta, $field) {
            $html = sprintf(
                    '<select class="rwmb-select-advanced" name="%s" id="%s" size="%s"%s data-options="%s">', $field['field_name'], $field['id'], $field['size'], $field['multiple'] ? ' multiple="multiple"' : '', esc_attr(json_encode($field['js_options']))
            );
