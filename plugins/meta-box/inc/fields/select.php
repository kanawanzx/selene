...
if (!class_exists('RWMB_Select_Field')) {
    ...
    class RWMB_Select_Field extends RWMB_Field {
        ...
        static function html($meta, $field) {
            $html = sprintf(
                    '<select class="rwmb-select" name="%s" id="%s" size="%s"%s>', $field['field_name'], $field['id'], $field['size'], $field['multiple'] ? ' multiple="multiple"' : ''
            );
        ...
        static function options_html($field, $meta) {
            ...
            foreach ($field['options'] as
                    ...
                    $label) {
                $html .= sprintf(
                        $option, $value, selected(in_array($value, (array) $meta), true, false), $label
                );
            }
