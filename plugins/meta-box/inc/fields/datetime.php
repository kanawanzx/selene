    ...
    class RWMB_Datetime_Field extends RWMB_Field {
        ...
        static function admin_enqueue_scripts() {
            ...
            wp_register_script('jquery-ui-timepicker-i18n', "{$url}/jquery-ui-timepicker-addon-i18n.min.js", array(
                'jquery-ui-timepicker'), '1.5.0', true);

            $date_paths = array(
                'jqueryui/datepicker-i18n/jquery.ui.datepicker-' . $locale . '.js');
            if (strlen($locale) > 2) {
            ...
            foreach ($date_paths as
                    $date_path) {
                if (file_exists(RWMB_DIR . 'js/' . $date_path)) {
                    wp_register_script('jquery-ui-datepicker-i18n', RWMB_JS_URL . $date_path, array(
                        'jquery-ui-datepicker'), '1.8.17', true);
        ...
        static function html($meta, $field) {
            return sprintf(
                    '<input type="text" class="rwmb-datetime" name="%s" value="%s" id="%s" size="%s" data-options="%s">', $field['field_name'], isset($field['timestamp']) && $field['timestamp'] ? date(self::translate_format($field), $meta) : $meta, isset($field['clone']) && $field['clone'] ? '' : $field['id'], $field['size'], esc_attr(json_encode($field['js_options']))
            );
        }
