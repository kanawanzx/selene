...
if (!class_exists('RWMB_Plupload_Image_Field')) {
    ...
    class RWMB_Plupload_Image_Field extends RWMB_Image_Field {
        ...
        static function add_actions() {
            parent::add_actions();
            add_action('wp_ajax_rwmb_plupload_image_upload', array(
                __CLASS__,
                'handle_upload'));
        }
        ...
        static function admin_enqueue_scripts() {
            ...
            wp_enqueue_style('rwmb-plupload-image', RWMB_CSS_URL . 'plupload-image.css', array(
                'wp-admin'), RWMB_VER);
            wp_enqueue_script('rwmb-plupload-image', RWMB_JS_URL . 'plupload-image.js', array(
                'jquery-ui-sortable',
                'wp-ajax-response',
                'plupload-all'), RWMB_VER, true);
            wp_localize_script('rwmb-plupload-image', 'RWMB', array(
                'url' => RWMB_URL));
        }
        ...
        static function html($meta, $field) {
            ...
            $html .= sprintf(
				...
				</div>', $field['id'], implode(' ', $classes), wp_create_nonce("rwmb-upload-images_{$field['id']}"), esc_attr(json_encode($field['js_options'])), $i18n_drop, $i18n_or, $field['id'], $i18n_select
            );
