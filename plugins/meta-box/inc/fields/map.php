    ...
    class RWMB_Map_Field extends RWMB_Field {
        ...
        static function html($meta, $field) {
            ...
            $html .= sprintf(
				...
				<input type="hidden" name="%s" class="rwmb-map-coordinate" value="%s">', isset($field['style']) ? $field['style'] : '', isset($field['std']) ? " data-default-loc=\"{$field['std']}\"" : '', $field['field_name'], $meta
            );

            if ($address) {
                $html .= sprintf(
                        '<button class="button rwmb-map-goto-address-button" value="%s">%s</button>', is_array($address) ? implode(',', $address) : $address, __('Find Address', 'meta-box')
                );
            }
