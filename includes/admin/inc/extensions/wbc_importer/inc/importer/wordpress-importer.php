...
if (class_exists('WP_Importer')) {
    ...
    class WP_Import extends WP_Importer {
        ...
        function get_author_mapping() {
            ...
            foreach ((array) $_POST['imported_authors'] as
                    $i =>
                    $old_login) {
                ...
                if (!empty($post['comments'])) {
                    ...
                    foreach ($newcomments as
                        ...
                        if (!$post_exists || !comment_exists($comment['comment_author'], $comment['comment_date'])) {
                            ...
                            foreach ($comment['commentmeta'] as
                                    $meta) {
            ...
            foreach ($this->post_orphans as
                ...
                if ($local_child_id && $local_parent_id)
                    $wpdb->update($wpdb->posts, array(
                        'post_parent' => $local_parent_id), array(
                        'ID' => $local_child_id), '%d', '%d');
            }

            // all other posts/terms are imported, retry menu items with missing associated object
            $missing_menu_items = $this->missing_menu_items;
            foreach ($missing_menu_items as
                    $item)
                $this->process_menu_item($item);

            // find parents for menu item orphans
            foreach ($this->menu_item_orphans as
                    $child_id =>
                    $parent_id) {
        ...
        function remap_featured_images() {
            ...
            foreach ($this->featured_images as
                    $post_id =>
                    $value) {
