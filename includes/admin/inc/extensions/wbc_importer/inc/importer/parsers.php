    ...
    function parse($file) {
        ...
        if (isset($result) && defined('IMPORT_DEBUG') && IMPORT_DEBUG) {
            ...
            if ('SimpleXML_parse_error' == $result->get_error_code()) {
                foreach ($result->get_error_data() as
                        $error)
...
class WXR_Parser_SimpleXML {
    ...
    function parse($file) {
        ...
        // grab authors
        foreach ($xml->xpath('/rss/channel/wp:author') as
                $author_arr) {
        ...
        foreach ($xml->xpath('/rss/channel/wp:category') as
                $term_arr) {
        ...
        foreach ($xml->xpath('/rss/channel/wp:tag') as
                $term_arr) {
            $t      = $term_arr->children($namespaces['wp']);
            $tags[] = array(
                'term_id'         => (int) $t->term_id,
        ...
        foreach ($xml->xpath('/rss/channel/wp:term') as
                $term_arr) {
            $t       = $term_arr->children($namespaces['wp']);
            $terms[] = array(
                'term_id'          => (int) $t->term_id,
